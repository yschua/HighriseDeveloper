...
bool GetUIElements(TiXmlNode* ParentNode, std::vector<sf::Drawable*>& ToFill)
{
    ...
    for (; gfxnode; gfxnode = gfxnode->NextSiblingElement()) {
        ...
        std::string Type = gfxnode->ValueStr();
        if (Type == "img") {
            TiXmlNode* file = gfxnode->FirstChild("file");
            if (file) {
                std::string filestr = file->FirstChild()->ValueStr();
            ...
            if (value) {
                std::string valuestr = value->FirstChild()->ValueStr();
            ...
            if (shapetypenode) {
                ShapeType = shapetypenode->FirstChild()->ValueStr();
            }
            if (ShapeType == "") {
                ...
                if (outline) {
                    ...
                    if (thikness) {
                        OutlineThikness = String::ToFloat(thikness->FirstChild()->ValueStr());
                    }
                }
                ...
                if (ShapeType == "rectangle") {
                    ...
                    std::cout << "creating rectangle at (" << pos.x << ", " << pos.y << ") with size ("
                              << size.x << ", " << size.y << ").\n";
                    *rect = sf::Shape::Rectangle(
                        pos.x, pos.y, size.x, size.y, FillColor, OutlineThikness, OutlineColor);
...
sf::Color GetColor(TiXmlNode* ColorNode)
{
    ...
    int a = 255;
    if (rnode)
        r = String::ToInt(rnode->FirstChild()->ValueStr());
    if (gnode)
        g = String::ToInt(gnode->FirstChild()->ValueStr());
    if (bnode)
        b = String::ToInt(bnode->FirstChild()->ValueStr());
    if (anode)
        a = String::ToInt(anode->FirstChild()->ValueStr());
...
sf::Vector2i GetPointi(TiXmlNode* PointNode)
{
    ...
    int y = 0;
    if (xnode)
        x = String::ToInt(xnode->FirstChild()->ValueStr());
    if (ynode)
        y = String::ToInt(ynode->FirstChild()->ValueStr());
...
sf::Vector2f GetPoint(TiXmlNode* PointNode)
{
    ...
    float y = 0;
    if (xnode)
        x = String::ToFloat(xnode->FirstChild()->ValueStr());
    if (ynode)
        y = String::ToFloat(ynode->FirstChild()->ValueStr());
