...
void ModelObject::Render(AnimationBase* pBase, bool bLight)
{
    ...
    // glBindTexture( GL_TEXTURE_2D, pBase->GetTextureID() );//to_draw.GetTexture() ); // get the current
    // texture
    pBase->BindTexture();
    if (bLight) {
...
void ModelObject::Render(AnimationBase* pBase, float x, float x2)
{
    float y = -pBase->GetPositionY(); //-ms.y;
...
void ModelObject::RenderParallax(FireAnimation* pBase,
...
void ModelObject::RenderFramework(AnimationBase* pBase,
...
void ModelObject::RenderFramework(AnimationBase* pBase,
                                  float x,
                                  float x2,
                                  int id) // overload with specified X and X2 and id
{
    float fx = x;
    float fy = -pBase->GetPositionY(); //-ms.y;
    float fz = 0;                      // pBase->GetPositionZ();
