...
void SettingsIni::SaveIni(std::string FileName, std::map<std::string, std::string>* Settings)
{
    ...
    for (std::map<std::string, std::string>::const_iterator iter = Settings->begin(); iter != Settings->end();
         ++iter) {
        ...
        if (DotIndex == std::string::npos)
            continue;
        std::string Setting = Section.substr(DotIndex + 1, Section.length() - DotIndex - 1);
    ...
    if (File.is_open()) {
        while (!File.eof()) {
            ...
            if (EqualOffset == std::string::npos)
                continue;
            std::string Setting = Section + '.' + Line.substr(Offset, (EqualOffset - Offset));
